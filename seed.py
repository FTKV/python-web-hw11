...
async def send_data_to_api() -> None:
    ...
    async for first_name, last_name, email, phone, birthday, address in get_fake_contact():
        data = {
        ...
        await session.post(
            "http://127.0.0.1:8000/api/contacts", data=json.dumps(data), headers=headers
        )
    await session.close()
    print("Done")
